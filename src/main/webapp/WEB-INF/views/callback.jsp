...
<body>
	<%
	System.out.println("콜백jsp");
		String clientId = "wIALdFClLRItjrr2Eb15";//애플리케이션 클라이언트 아이디값";
	String clientSecret = "jrhvpHkliC";//애플리케이션 클라이언트 시크릿값";
	String code = request.getParameter("code");
	String state = request.getParameter("state");
	String redirectURI = URLEncoder.encode("http://localhost:8080/board/callback", "UTF-8");
	String apiURL;
	...
	try {
		if (session.getAttribute("state") != null && session.getAttribute("state").equals(state)) {
			...
			if (responseCode == 200) {
				...
				RequestDispatcher rd = request.getRequestDispatcher("/naverlogin");
